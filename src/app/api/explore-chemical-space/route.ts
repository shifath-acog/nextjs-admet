...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch('http://molecular-property-prediction:8000/explore-chemical-space/', {
      method: 'POST',
      body: formData,
    });
    ...
    console.error('Chemical space error:', error);
    return NextResponse.json({ error: 'Failed to explore chemical space' }, { status: 500 });
  }
}